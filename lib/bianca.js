...
function expectResult(value) {
  ...
  function toThrowError (secondValue) {
    var error = { message: 'function not given'}
    if (typeof this.val !== 'function') { throw error }
    try { this.val() } catch(e) { return true }
    error.message = "no error thrown";
    throw error;
  };

  function toNotThrowError (secondValue) {
    var error = { message: 'function not given'}
    if (typeof this.val !== 'function') { throw error }
    try { this.val() } catch(e) {
      fnName = this.val.name || "unnamed function"
      error.message = "error thrown by " + fnName;
      throw error;
    }
    return true
  };
